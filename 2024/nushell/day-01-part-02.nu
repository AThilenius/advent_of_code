...
export def main [
  ...
  path: path = "../puzzle-inputs/01-historian-hysteria.txt",
] {
  let lines = open $path
    | lines
    | parse "{left}   {right}"
  ...
  # Convert right into a record (hashmap) of id -> count of occurrences. This
  # will keep the computational-complexity O(N)
  let counts = $lines
    | get right
    | group-by --to-table
    | each { { $in.group: ($in.items | length) } }
    | into record

  $lines
    | get left
    | each { |id| ($id | into int) * ($counts | default 0 $id | get $id) }
    | math sum
}
