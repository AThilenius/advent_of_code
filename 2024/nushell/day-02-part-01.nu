...
# --- Day 2: Red-Nosed Reports --- Fortunately, the first location The
# Historians want to search isn't a long walk from the Chief Historian's
# office.
#
...
export def main [
  ...
  open $path
    | lines
    # Convert to the difference between each set of 2 numbers
    | each { split row " " | into int | window 2 | each { $in.1 - $in.0 } }
    # Convert to true/false based on if the cells are all increasing by 1..3
    # or 'increasing' by -1..-3
    | each { |deltas| 
